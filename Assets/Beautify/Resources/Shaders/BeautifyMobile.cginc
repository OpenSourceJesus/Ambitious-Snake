	...
	void beautifyPassFast(v2f i, inout float3 rgbM) {
		...
		float  CoC        = getCoc(i.depthUV);
		       dofPix.a   = saturate(min(CoC, dofPix.a));
		       rgbM       = lerp(rgbM, dofPix.rgb, dofPix.a);
		#endif
		...
		rgbM = LinearToGammaSpace(rgbM);
 	 	...
 	 	// 6. Lens Dirt
		#if DIRT
		float  curLum     = tex2D(_ScreenLum, halves.xy).a;
   	 	float  scrLum     = saturate((curLum - _Dirt.z) * _Dirt.y);
